...
<body>
    <div class="p-3">
        <h2 class="text-center mb-4">Registro de bitácoras</h2>
        <form action="{{route('regBitacora')}}" method="post" class="row g-3">
            @csrf
            @method('POST')
            <div class="col-md-6">
                <label for="tipo_trabajo" class="form-label">Tipo de trabajo</label>
                <select name="tipo_trabajo" change="tptrab" id="tptrab" class="form-select">
      ...
      ?>
            ...
            <div class="col-md-6">
                ...
                <select name="rama" id="rama" change="mat" class="form-select">
      ...
      ?>
            ...
            <div class="col-12">
                ...
                <select id="" name="lider" class="form-select">
                    <?php
                    foreach ($lideres as $lider) {
            echo '<option value="'.$lider->id_lid.'">'.$lider->nombre_lid.'</option>';
                    }
    ...
    ?>
            ...
            <div class="col-md-6">
                ...
                <select id="estados" name="estados" onchange="cargarCiudades();" class="form-select">
            ...
            <div class="col-12 ms-3">
                <input class="form-check-input d-none" type="checkbox" id="material-select-all">
                <label class="form-check-label d-none" for="material-select-all"
                    id="material-select-all-label">Seleccionar todos</label>
            </div>
            <div class="col-12 ms-3">
                <div class="form-check" id="material">
                </div>
                <div class="col-12 text-center mt-4" id="regbit">
                    <button type="submit" class="btn btn-outline-dark align-item-center" id="btn-reg">Registrar
                        bitacora</button>
                </div>
        </form>
    </div>
...
<div class="website-counter"></div> -->
   ...
   <script type="text/javascript" async>
    ...
    $("#tptrab").change(function tp() {
    $("#material").empty();
    $("#material-select-all").prop("checked", false);
    $("#material-select-all").addClass("d-none");
    $("#material-select-all-label").addClass("d-none");
    $("#rama").change(function mat(){
    $("#material").empty();
    var tipotrabajo = document.getElementById("tptrab").value;
    if(tipotrabajo != '5' && tipotrabajo != '6' && tipotrabajo !='4'){
    ...
    $("#material").append($("<h4>").text("No existe ningún material registrado para esta rama"));
      ...
      $('#material').append(
      '<input class="form-check-input" type="checkbox" name="mats[]" value='+ item.id+'>' + item.nombre + '<br>'
      );
      }
      });
    
      // toggle the visibility of the select all checkbox based on the number of checkboxes present
      var checkboxes = $("#material input[type='checkbox']");
      if (checkboxes.length > 1) {
        $("#material-select-all").removeClass("d-none");
        $("#material-select-all-label").removeClass("d-none");
      } else {
      $("#material-select-all").addClass("d-none");
      $("#material-select-all-label").addClass("d-none");
      }
      ...
      // toggle the selection of all checkboxes when the select all button is clicked
      $("#material-select-all").change(function() {
      $("input[name='mats[]']").prop('checked', $(this).prop("checked"));
      });
      });
    </script>
...
function addOptions(domElement, array) {
    ...
    for (estados in array) {
        ...
        // Añadimos un value a los option para hacer mas facil escoger los ciudades
...
function cargarCiudades() {
    ...
    ciudades.innerHTML = '<option disabled value="">Selecciona una ciudad</option>'
