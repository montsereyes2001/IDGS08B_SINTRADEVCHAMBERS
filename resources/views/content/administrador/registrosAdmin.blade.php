...
<script type="text/javascript">
    $(document).ready(function (){
        function search(){
            ...
            var correoLid = document.getElementById("lider").value.toString();
            console.log(correoLid);
                ...
                success: function(data){
                    ...
                    data.forEach(item => {
                        ...
                        $(".row").append(
                            ...
                            $("<div>").addClass("fs-3").text(trab)).append($("<div>").text(item.ciudad+", "+item.estado)).append('<p>Iniciada el: '+item.created_at+' <br> Modificada el: '+item.updated_at+'</p>').append(progress).append($("<div>").addClass("text-center").append($("<a>").addClass('btn btn-outline-primary mt-3').attr({href:"../admin/gallery/"+item.id, style:"margin-right: 2% !important;", id:"botongal"}).text("Galeria"))
                                //Aquí faltan las funciones y las rutas web para el administrador
                          .append($('<a>').addClass('btn btn-outline-warning mt-3').attr({href:"../admin/bitview/mod/"+item.id, id:"botoned"} ).text("Modificar")).append('<form  action="<?php echo env("URL")."admin/bit/del/"?>'+item.id+'" method="post">@csrf @method("PUT")<input type="hidden" name="Inactivo" id="inactivo"> <button class ="btn btn-outline-danger mt-3" type="submit" id="btndel">Eliminar</button></form>')))))) ); 
