...
<body>
    @php
    // Retrieve the value of the "source" parameter from the URL
    $source_view = isset($_GET['source']) ? $_GET['source'] : '';
    
    // Set the URL based on the source view
    if ($source_view === 'view1') {
   $url = env('URL').'owner/trabajos/'.$bitacora->trab.'';
    } else {
    $url = env('URL').'owner/registros';
    }
    @endphp
     <div class="ms-3 mt-3 text-start container">
    <a class="btn btn-outline-dark align-item-center" href="{{ $url }}">
        <span class="ms-1 d-sm-inline bi bi-arrow-bar-left fs-4"></span>&nbsp;Regresar
    </a>
    </div>
    ...
    <div class="ms-5 mt-3">
        ...
        <div class='ms-3 mt-3 text-end me-2'>
            <div class="d-inline-flex">
                ...
                @elseif($bitacora->trab=="Distribución")
                <form action='{{route("kml")}}' method="post">
                    @csrf
                    @method('post')
                    <?php
            ...
            <div class="col-12 col-md-6 col-lg-3 carta">
                <div class="card mb-2">
                    ...
                    <div class="card-body">
                        ...
                        <a href="'.route('EvidenciaOwner', $ev->id).'" class="btn btn-outline-warning" style="color: yellow;">Ver
                            más</a>
                    </div>
                </div>
            </div>
